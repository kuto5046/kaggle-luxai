...
class ImitationAgent(Agent):
    ...
    def process_unit_turn(self, game, team):
        ...
        policy_map = self.tta.apply(obs, global_obs)
...
class TTA:            
    ...
    def apply(self, obs, global_obs):

        policy_map1 = self.torch_predict(obs, global_obs)
        policy_map2 = self.tta.horizontal_convert_action(self.torch_predict(self.tta.horizontal_flip(obs), global_obs))
        policy_map3 = self.tta.vertical_convert_action(self.torch_predict(self.tta.vertical_flip(obs), global_obs))
        policy_map4 = self.tta.all_convert_action(self.torch_predict(self.tta.all_flip(obs), global_obs))
        ...
        policy_map = np.mean([policy_map1, policy_map2, policy_map3, policy_map4], axis=0)
        return policy_map