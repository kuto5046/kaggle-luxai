...
class ImitationAgent(Agent):
    ...
    def process_unit_turn(self, game, team):
        ...
        # policy_map1 = self.onnx_predict(obs, global_obs)
        # policy_map2 = self.tta.horizontal_convert_action(self.onnx_predict(self.tta.horizontal_flip(obs), global_obs))
        # policy_map3 = self.tta.vertical_convert_action(self.onnx_predict(self.tta.vertical_flip(obs), global_obs))
        # policy_map4 = self.tta.all_convert_action(self.onnx_predict(self.tta.all_flip(obs), global_obs))

        # policy_map5 = self.tta.reverse_random_roll(self.onnx_predict(self.tta.random_roll(obs), global_obs))
        # policy_map6 = self.tta.reverse_random_roll(self.onnx_predict(self.tta.random_roll(obs), global_obs))
        # policy_map7 = self.tta.reverse_random_roll(self.onnx_predict(self.tta.random_roll(obs), global_obs))
        # policy_map8 = self.tta.reverse_random_roll(self.onnx_predict(self.tta.random_roll(obs), global_obs))
