    ...
    def process_unit_turn(self, game, team, base_obs):
        ...
        for unit in units.values():
            if unit.can_act():
                ...
                # policy2, value2 = self.onnx_predict(self.tta.vertical_flip(obs))
                # policy2 = self.tta.vertical_convert_action(policy2)
                # policy3, value3 = self.onnx_predict(self.tta.horizontal_flip(obs))
                # policy3 = self.tta.horizontal_convert_action(policy3)
                # policy4, value4 = self.onnx_predict(self.tta.all_flip(obs))
                # policy4 = self.tta.all_convert_action(policy4)
                # policy5, value5 = self.onnx_predict(self.tta.random_roll(obs))
                # policy6, value6 = self.onnx_predict(self.tta.random_roll(obs))
                # policy = np.mean([policy1, policy2, policy3, policy4, policy5, policy6], axis=0)
                # value = np.mean([value1, value2, value3, value4, value5, value6], axis=0)
