...
class CustomMlpExtractor(nn.Module):
    ...
    def forward(self, features):
        ...
        return self.policy_net(features[0][features[2] > 0].view(batch_size, 6)), self.value_net(features[1])

    def forward_actor(self, features):
        batch_size = features[0].shape[0]
        return self.policy_net(features[0][features[2] > 0].view(batch_size, 6))
...
class ImitationAgent(Agent):
    def __init__(self, model=None) -> None:
        ...
        self.observation_space = spaces.Dict(
            ...
            "mask":spaces.Box(low=0, high=1, shape=(32, 32), dtype=np.long),
            })
        self.model = model
