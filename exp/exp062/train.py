...
class CustomCheckpointCallback(CheckpointCallback):
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            # copy_model2 = copy.deepcopy(self_play_model2)  # cpuで保存すると元のmodelもcpuになってしまうのでdeepcopyする
            torch.onnx.export(copy_model.cpu(), {"obs": dummy_obs, "global_obs": dummy_global_obs, "mask": dummy_mask}, f"{path}_all.onnx", input_names=["obs", "global_obs", "mask"], opset_version=11)
            # torch.onnx.export(copy_model2.cpu(), {"obs": dummy_obs, "global_obs": dummy_global_obs, "mask": dummy_mask}, f"{path}_one.onnx", input_names=["obs", "global_obs", "mask"], opset_version=11)
...
def main():
    ...
    if os.path.exists(pretrained_rl_path):
        ...
        class BCPolicy(CustomActorCriticCnnPolicy):
            def __new__(cls, *args, **kwargs):
                policy = bc.reconstruct_policy(pretrained_il_path)                
                return policy
    ...
    # for param in model.policy.features_extractor.outc.parameters():
